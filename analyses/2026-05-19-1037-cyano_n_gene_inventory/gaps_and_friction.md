...
**Workaround.** Initially: accept and document (tier purity preferred). Researcher overrode that — *"why do you lose? don't throw out the orphans"*. Switched to recovery via `06_orphan_recovery.py`: each (strain, gene_name) orphan combination became a synthetic singleton group with `group_id = anchor_singleton:<gene>:<strain>`, `source = anchor_singleton`. 9 orphan loci became 7 synthetic groups; inventory matrix grew 19 × 54 → 19 × 61; the 6 previously-undercounted cells are now correctly populated. Tier-mixing tolerated; the `source` column on `04_ortholog_groups.csv` distinguishes `eggnog` (cross-strain ortholog) from `anchor_singleton` (strain-specific anchor presence). Downstream analysis can use the `source` flag to handle each class appropriately.
