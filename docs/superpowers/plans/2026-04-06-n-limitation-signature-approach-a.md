...
def summarize_de_per_gene(de_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        ...
        peak_log2fc, peak_timepoint, best_dir_rank, best_global_rank.
    """
    ...
    # Pick peak timepoint (max |log2fc|) in majority direction
    sig = sig.merge(majority_direction, on="locus_tag")
    sig_majority = sig[sig["direction"] == sig["majority_direction"]]

    peak = sig_majority.loc[sig_majority.groupby("locus_tag")["abs_log2fc"].idxmax()]
    ...
    result = peak[["locus_tag", "gene_name", "direction", "log2fc", "timepoint"]].copy()
    result = result.rename(columns={"log2fc": "peak_log2fc", "timepoint": "peak_timepoint"})
...
def intersect_de_lists(
    ...
    Returns:
        (core_df, extended_df) — each with columns:
            ...
            study_a_peak_log2fc, study_a_peak_timepoint, study_a_best_dir_rank, study_a_best_global_rank,
            study_b_peak_log2fc, study_b_peak_timepoint, study_b_best_dir_rank, study_b_best_global_rank,
    ...
    cols = [
        ...
        "study_a_peak_log2fc", "study_a_peak_timepoint",
        "study_a_best_dir_rank", "study_a_best_global_rank",
        "study_b_peak_log2fc", "study_b_peak_timepoint",
